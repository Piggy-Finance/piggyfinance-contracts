...
contract PiggyRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PIGGYs to distribute in the pool.
        uint256 lastRewardTime; // Last time that PIGGYs distribution occurred.
        uint256 accPiggyPerShare; // Accumulated PIGGYs per share, times 1e18. See below.
    ...
    uint256[3] public epochPiggyPerSecond;
    ...
    constructor(address _piggy, uint256 _poolStartTime) public {
        ...
        epochPiggyPerSecond[0] = epochTotalRewards[0].div(4 days);
        epochPiggyPerSecond[1] = epochTotalRewards[1].div(5 days);

        epochPiggyPerSecond[2] = 0;
        operator = msg.sender;
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "PiggyRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "PiggyRewardPool: existing pool?");
        }
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accPiggyPerShare: 0, isStarted: _isStarted}));
        if (_isStarted) {
        ...
        for (uint8 epochId = 2; epochId >= 1; --epochId) {
            if (_toTime >= epochEndTimes[epochId - 1]) {
                if (_fromTime >= epochEndTimes[epochId - 1]) {
                    return _toTime.sub(_fromTime).mul(epochPiggyPerSecond[epochId]);
                }

                uint256 _generatedReward = _toTime.sub(epochEndTimes[epochId - 1]).mul(epochPiggyPerSecond[epochId]);
                if (epochId == 1) {
                    return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochPiggyPerSecond[0]));
                }
                for (epochId = epochId - 1; epochId >= 1; --epochId) {
                    if (_fromTime >= epochEndTimes[epochId - 1]) {
                        return _generatedReward.add(epochEndTimes[epochId].sub(_fromTime).mul(epochPiggyPerSecond[epochId]));
                    }
                    _generatedReward = _generatedReward.add(epochEndTimes[epochId].sub(epochEndTimes[epochId - 1]).mul(epochPiggyPerSecond[epochId]));
                }
                return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochPiggyPerSecond[0]));
            }
        }
        return _toTime.sub(_fromTime).mul(epochPiggyPerSecond[0]);
    }

    // View function to see pending PIGGYs on frontend.
    ...
    function safePiggyTransfer(address _to, uint256 _amount) internal {
        uint256 _piggyBal = piggy.balanceOf(address(this));
        if (_piggyBal > 0) {
            if (_amount > _piggyBal) {
                piggy.safeTransfer(_to, _piggyBal);
            } else {
