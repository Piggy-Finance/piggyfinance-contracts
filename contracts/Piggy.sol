...
*/
contract Piggy is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the PIGGY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("PIGGY", "PIGGY") {
        // Mints 1 PIGGY to contract creator for initial pool setup
    ...
    function _getPiggyPrice() internal view returns (uint256 _piggyPrice) {
        try IOracle(piggyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Piggy: failed to fetch PIGGY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _piggyPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_piggyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPiggyOracle(address _piggyOracle) public onlyOperatorOrTaxOffice {
        require(_piggyOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints PIGGY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PIGGY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPiggyPrice = _getPiggyPrice();
            currentTaxRate = _updateTaxRate(currentPiggyPrice);
            if (currentPiggyPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        _mint(_genesisPool, INITIAL_LAUNCH_DISTRIBUTION);
    }
